...
class ChunkRecord:
    ...
    wav_file: str
    mp3_file: str
    spoken_text_file: str = ""
    text_length: int = 0
    spoken_text_length: int = 0
    chapter_index: int = 1
    chapter_title: str = ""
...
class JobState:
    ...
    cached_completed_chunks: int = 0
    cached_total_chunks: int = 0
    cached_failed_chunks: int = 0
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "JobState":
        ...
        for chunk in payload.get("chunks", []):
            ...
            chunk.setdefault("text_length", 0)
            chunk.setdefault("spoken_text_file", "")
    ...
    def completed_chunks(self) -> int:
        if not self.chunks and self.cached_total_chunks:
            return int(self.cached_completed_chunks)
        return sum(1 for chunk in self.chunks if chunk.status == "done")

    @property
    def total_chunks(self) -> int:
        if not self.chunks and self.cached_total_chunks:
            return int(self.cached_total_chunks)
        return len(self.chunks)

    def append_log(self, message: str) -> None:
        self.logs.append(f"{utc_now()} {message}")
        if len(self.logs) > MAX_PERSISTED_JOB_LOGS:
            self.logs = self.logs[-MAX_PERSISTED_JOB_LOGS:]
